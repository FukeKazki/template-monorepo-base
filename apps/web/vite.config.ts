...
export default defineConfig(({ mode }) => {
  // .env / .env.local などから USE_MOCK・VITE_API_PROXY_TARGET を読み込む（第3引数を ""
  // にすることで VITE_ prefix 以外の変数もロードする）。process.env の値が優先される。
    ...
    plugins: [
      ...
      tailwindcss(),
      {
        ...
        transformIndexHtml: (html: string) =>
          env["USE_MOCK"] === "false" ? html : html.replace("/src/main.tsx", "/src/dev-entry.ts"),
      },
    ],
    resolve: {
      alias: {
